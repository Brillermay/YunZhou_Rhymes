...
public class UserAdd extends ConnetMySQL {

    @PostMapping("/add")
    @Operation(summary = "添加用户，传递User对象")
    public String addUser(@RequestBody User user) throws IOException {
        try {
            InputStream in = Resources.getResourceAsStream("SqlMapConfig.xml");
            SqlSession session = getSession(in);
            UserAddMapper userAddMapper = session.getMapper(UserAddMapper.class);
            int result = userAddMapper.UserAdd(user);
            session.commit();
            in.close();
            session.close();
