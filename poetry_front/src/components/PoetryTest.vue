    ...
    <div id="mtapp">
      <div class="container">
        <div class="bg">
            ...
            <div v-if="isModalVisible" class="modal-overlay">
              <div class="modal">
                ...
                <template v-if="showResultsFlag">
                  <h2 v-if="correctCount <= 5">诗词萌新</h2>
                  <h2 v-else-if="correctCount <= 7">诗词小达人</h2>
                  <h2 v-else>诗词通关王</h2>
          ...
          <!-- 当前题目显示 -->
          <div class="test">
            <div class="a" v-if="currentQuestion">
              ...
              <div class="option">
                <button
                  ...
                  @click="handleOptionClick(currentQuestion.id, key)"
                  :disabled="showResultsFlag || isAnswered"
                >
                  {{ optLabel(key) }}.{{ text }}
                </button>
              </div>
           ...
           <div class="progress-bar-bg">
            ...
            <img
      src="../assets/image/imgtest/brush.png"
      alt="毛笔"
      class="brush"
...
/* 弹窗容器 */
.modal {
  background: linear-gradient(135deg, #f6df8e, #c9c19e); /* 古风米白+淡灰 */
  padding: 30px 40px;
  border-radius: 15px;
  text-align: center;
  width: 350px;
...
.modal button {
  background: linear-gradient(to right, #aad1b2, #e3af88); /* 墨绿到木棕 */
  color: #f9f5ec; /* 柔和米白字色 */
  border: none;
  padding: 12px 25px;
  border-radius: 50px;
...
.modal button:hover {
  background: linear-gradient(to right, #9ac7a4, #141312); /* hover 时略深 */
...
.modal h2 {
  font-size: 27px;
  font-weight: 600;
  color: linear-gradient(45deg, #ff6f61, #f8b400);
...
.modal p {
  ...
  word-break: break-word;   /* 强制长单词或 URL 换行 */
...
/* 按钮点击效果 */
.modal button:active {
  transform: translateY(1px); /* 点击时向下 */
